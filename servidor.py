...
import zlib
import cv2
import pyautogui  # Biblioteca para controle do mouse e teclado
...
def handle_client(conn):
    ...
    try:
        while True:
            ...
            command = data.decode()
            if command == 'LEFT':
                pyautogui.moveRel(-10, 0)  # Move o cursor 10 pixels para a esquerda
            elif command == 'RIGHT':
                pyautogui.moveRel(10, 0)  # Move o cursor 10 pixels para a direita
            elif command == 'UP':
                pyautogui.moveRel(0, -10)  # Move o cursor 10 pixels para cima
            elif command == 'DOWN':
                pyautogui.moveRel(0, 10)  # Move o cursor 10 pixels para baixo
    except Exception as e:
